...
public class MonitorProperties {
    ...
    private static void getProperties() {
        ...
        try {
            File f = null;
            f = new File("C:/properties/ca.properties");
            if (!f.exists()) {
                f = new File("/opt/properties/ca.properties");
            }
            if (!f.exists()) {
                logger.log(Level.SEVERE, "CA Properties File not found");
            } else {
                logger.log(Level.INFO, "CA Properties: {0}\n\n\n", f.getAbsolutePath());
                props.load(new FileInputStream(f));
            }
        } catch (IOException e) {
            logger.log(Level.SEVERE, "Properties file ca.properties not found or corrupted");
        }
